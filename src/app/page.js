...
const GOOGLE_MAPS_URL = 'https://www.google.com/maps/place/Titan+Sports+Arena/@20.9438211,77.8016039,17.59z'
    ...
    <>
      ...
      <section className="relative flex items-center overflow-hidden" style={{ height: '100vh', minHeight: 620 }}>
        <div className="absolute inset-0">
          <img src="/images/turf-night.png" alt="Titan Sports Arena at night" className="w-full h-full object-cover scale-[1.12]" style={{ objectPosition: 'center 40%' }} />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 w-full" style={{ paddingTop: 80 }}>
          <div className="max-w-2xl">
            <div className="flex items-center gap-3 mb-6" style={{ animation: 'revealUp 0.9s ease 0.1s both' }}>
              <span className="text-xs font-bold tracking-widest uppercase text-white px-3 py-1" style={{ background: '#4a8c4a' }}>FIFA-Approved Turf</span>
      ...
      <section className="py-24 px-6" style={{ background: 'var(--titan-cream)' }}>
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <Reveal delay={0.15} className="relative" style={{ paddingBottom: 40, paddingRight: 16 }}>
            ...
            <div className="absolute bg-white p-5" style={{ bottom: -8, left: -16, maxWidth: 180, boxShadow: '0 12px 40px rgba(0,0,0,0.12)' }}>
      ...
      <section className="py-24 px-6" style={{ background: 'white' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { img: '/images/turf-cricket.png', tag: 'Cricket', title: 'Box Cricket', desc: 'Professional pitch with stumps and crease markings. Perfect for competitive or casual cricket on a premium FIFA-grade surface.', href: '/facilities', scaleClass: 'scale-[1.3]' },
              { img: '/images/turf-night.png', tag: 'Football', title: 'Football', desc: 'Full-size goals, penalty spots, and ample space for 5-a-side or larger matches under state-of-the-art floodlights.', href: '/facilities', pos: 'center 40%', scaleClass: 'scale-[1.15]' },
              { img: '/images/cafe-interior.png', tag: 'Experience', title: 'Mini Machines Café', desc: "Amravati's first sports-themed café with RC cars, board games, live screen, and great food & drinks.", href: '/cafe', scaleClass: 'scale-[1.2]' },
            ].map((c, i) => (
              <Reveal key={c.title} delay={i * 0.1}>
                <div className="sport-card">
                  <div className="sport-card-img overflow-hidden" style={{ height: 230 }}>
                    <img src={c.img} alt={c.title} className={`w-full h-full object-cover ${c.scaleClass || ''}`} style={{ objectPosition: c.pos || 'center' }} />
                  </div>
                  <div className="p-7">
                    <span className="inline-block text-xs font-bold tracking-widest uppercase text-white mb-3 px-2 py-1" style={{ background: '#4a8c4a' }}>{c.tag}</span>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2" style={{ minHeight: 500 }}>
        <div className="relative overflow-hidden" style={{ minHeight: 360 }}>
          <img src="/images/cafe-interior.png" alt="Mini Machines Café" className="absolute inset-0 w-full h-full object-cover scale-[1.2]" />
          <div className="absolute inset-0" style={{ background: 'rgba(13,26,13,0.2)' }} />
        </div>
      ...
      {/* ── REVIEWS ── All cards link to Google Maps ── */}
      <section className="py-24 px-6" style={{ background: 'var(--titan-cream)' }}>
        <div className="max-w-7xl mx-auto">
          <Reveal className="text-center mb-14">
            <div style={{ width: 44, height: 3, background: '#c0392b', margin: '0 auto 20px' }} />
            ...
            {reviews.map((r, i) => (
              <Reveal key={r.name} delay={i * 0.07}>
                <a href={GOOGLE_MAPS_URL} target="_blank" rel="noopener noreferrer"
                  className="block p-7 bg-white no-underline group transition-all hover:-translate-y-[3px] hover:border-l-[#c0392b] hover:shadow-[0_12px_32px_rgba(0,0,0,0.1)]"
                  style={{ borderLeft: '3px solid transparent', boxShadow: '0 1px 8px rgba(0,0,0,0.06)', transitionProperty: 'border-color, transform, box-shadow', transitionDuration: '0.25s' }}>
                  ...
                  <p className="text-xs mt-2 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: '#c0392b', fontWeight: 600 }}>View on Google Maps →</p>
                </a>
              </Reveal>
            ))}
          </div>
      ...
      {/* ── CTA BANNER ── */}
      <section className="relative overflow-hidden" style={{ padding: '120px 24px' }}>
        <div className="absolute inset-0">
          <img src="/images/turf-aerial.png" alt="Turf" className="w-full h-full object-cover" style={{ objectPosition: 'center 55%' }} />
          <div className="absolute inset-0" style={{ background: 'rgba(13,26,13,0.48)' }} />
        </div>
        <Reveal className="relative z-10 max-w-2xl mx-auto text-center text-white">
          <h2 className="font-display leading-none mb-4" style={{ fontSize: 'clamp(3rem,8vw,5.5rem)' }}>READY TO PLAY?</h2>
