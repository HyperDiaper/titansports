    ...
    <>
      <section className="relative pt-36 pb-24 px-6 overflow-hidden" style={{ background: '#0d1a0d' }}>
        <div className="absolute inset-0">
          <img src="/images/cafe-interior.png" alt="Café" className="w-full h-full object-cover opacity-35" />
          <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom,rgba(13,26,13,0.87),rgba(13,26,13,0.7))' }} />
        </div>
        <div className="relative z-10 max-w-4xl mx-auto text-center text-white">
          <Reveal><img src="/images/cafe-logo.png" alt="Mini Machines Café" className="w-24 h-24 object-contain mx-auto mb-6 rounded-full" /></Reveal>
      ...
      <section className="py-20 px-6" style={{ background: 'white' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              { img: '/images/rc-cars.png', tag: 'Activity', title: 'RC Car Racing', desc: 'Race remote-controlled cars on a dedicated embedded track. Compete with friends for speed and control.' },
              { img: '/images/rc-excavator.png', tag: 'Activity', title: 'RC Excavators & Trucks', desc: 'Operate full-scale RC construction vehicles on a real sand pit — a unique, hands-on experience for all ages.' },
              { img: '/images/board-games.png', tag: 'Games', title: 'Board Games', desc: 'Challenge someone to chess, carrom, Jenga, and more board games in a relaxed café setting.' },
              { img: '/images/cafe-interior.png', tag: 'Entertainment', title: 'Live Sport Screening', desc: 'Watch live cricket, football, and other sports on the large projection screen with great sound.' },
              { img: '/images/cafe-side.png', tag: 'Experience', title: 'Themed Ambiance', desc: 'Stunning hand-painted graffiti murals, industrial-chic decor, and dramatic lighting make every visit memorable.' },
              { img: '/images/cafe-interior2.png', tag: 'Seating', title: 'Comfortable Seating', desc: 'Spacious group tables, bar-height stools, and cosy corners for every type of gathering.' },
            ].map((f, i) => (
              <Reveal key={f.title} delay={i * 0.07}>
                <div className="sport-card">
                  <div className="sport-card-img overflow-hidden" style={{ height: 200 }}>
                    <img src={f.img} alt={f.title} className="w-full h-full object-cover" />
                  </div>
                  <div className="p-6">
                    <span className="inline-block text-xs font-bold tracking-widest uppercase text-white mb-3 px-2 py-1" style={{ background: '#4a8c4a', borderRadius: 2 }}>{f.tag}</span>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Reveal className="lg:col-span-2">
            <div className="overflow-hidden" style={{ height: 380 }}>
              <img src="/images/cafe-interior.png" alt="Café ambiance" className="w-full h-full object-cover" />
            </div>
          </Reveal>
      ...
      {/* CTA */}
      <section className="py-20 px-6 text-center" style={{ background: '#f5f0e8' }}>
